...
class Financiero extends Component
{
    ...
    public function mount()
    {
        ...
        // ->where('status', 6)
        // ->get();
        $totalVentas = Order::whereHas('items', function ($query) {
            $query->where('instructor_id', auth()->user()->id);
        })
            ->sum('total');

        $totalDevolucions=Devolucion::whereHas('items', function ($query) {
            $query->where('instructor_id', auth()->user()->id);
        })
            ->where('status', 1)
            ->sum('total');
        ...
        $this->totalSaldo = number_format(SaldoFavor::where('instructor_id', auth()->user()->id)
        ->where('status', 1)
        ...
        $this->transactions = Transaction::where('instructor_id', auth()->user()->id)->get();
    }
    ...
    public function render()
    {
        ...
        return view('livewire.author.financiero', [
