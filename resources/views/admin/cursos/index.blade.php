    ...
    <div class="bg-white rounded-[2rem] shadow-xl shadow-gray-200/50 border border-gray-100 overflow-hidden relative">
        ...
        <div class="px-8 py-6 bg-[#335A92] text-white flex flex-col sm:flex-row justify-between items-center gap-4 relative overflow-hidden">
             ...
             <div class="relative z-10 flex items-center gap-4">
                 ...
                 <div>
                    <h2 class="text-2xl font-bold tracking-tight">Cursos Pendientes</h2>
                    <p class="text-blue-100 text-sm font-medium">Revisión y aprobación de contenido académico</p>
                 </div>
             </div>
             
             <div class="relative z-10 flex items-center gap-3">
                 <div class="px-4 py-2 bg-white/10 rounded-xl backdrop-blur-sm border border-white/20 text-white text-xs font-bold">
                     <i class="fas fa-clock mr-1 text-[#ECBD2D]"></i>
                     {{ $courses->count() }} Pendientes
                 </div>
                 <a href="{{ route('admin.cursos.noaprobado', 1) }}" class="px-4 py-2.5 bg-red-500 hover:bg-red-600 text-white font-bold rounded-xl shadow-lg transition-all flex items-center text-sm">
                    <i class="fas fa-ban mr-2"></i> Rechazados
                 </a>
             </div>
        </div>
        ...
        <div class="overflow-x-auto">
                        ...
                        @foreach ($courses as $course)
                            <tr class="bg-white even:bg-[#335A92]/5 hover:bg-[#477EB1]/10 transition duration-200 group">
                                ...
                                <td class="px-8 py-5 text-right">
                                    <a href="{{ route('admin.cursos.show', $course) }}" class="inline-flex items-center px-5 py-2.5 bg-[#335A92] border border-transparent rounded-xl font-bold text-xs text-white uppercase tracking-widest hover:bg-[#284672] active:bg-[#1a2e4b] focus:outline-none transition ease-in-out duration-150 shadow-lg shadow-blue-900/20 transform hover:-translate-y-0.5">
            ...
            @else
                <div class="p-16 text-center text-gray-500">
                    <div class="bg-green-50 rounded-full w-24 h-24 flex items-center justify-center mx-auto mb-6 shadow-sm border border-green-100">
                        <i class="fas fa-check-circle text-5xl text-green-500"></i>
                    </div>
                    <h3 class="text-xl font-bold text-gray-900 mb-2">¡Todo al día!</h3>
                    <p class="text-gray-500">No hay cursos pendientes de aprobación en este momento.</p>
                </div>
            @endif
        </div>
    </div>
