<x-admin-layout>
    <div class="container mx-auto px-6 py-8" x-data="{ openCreateModal: false, openEditModal: false, editAction: '', editName: '', editPublic: false }">
            ...
            <div class="p-6 border-b border-gray-100 dark:border-gray-600 bg-white/50 dark:bg-gray-800/50 backdrop-blur-sm relative z-10 flex flex-col sm:flex-row justify-between items-center gap-4">
                <div>
                    <h2 class="text-xl font-bold text-gray-800 dark:text-white flex items-center">
                        <i class="fas fa-layer-group text-primary-600 mr-2 bg-primary-50 p-2 rounded-lg"></i>
                        <span class="tracking-tight">Mis Categorías</span>
                    </h2>
                    <p class="text-xs text-gray-500 dark:text-gray-400 mt-1 ml-10">Gestiona las categorías de cursos para: <span class="font-bold text-primary-600">{{ $empresa->nombre }}</span></p>
                </div>
                        ...
                        <tbody>
                            @foreach ($categorias as $categoria)
                                <tr class="bg-white border-b dark:bg-gray-800 dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-600">
                                    ...
                                    <td class="px-6 py-4">{{ $categoria->slug }}</td>
                                    <td class="px-6 py-4">
                                    ...
                                    <td class="px-6 py-4 text-center space-x-2">
                                        ...
                                        <form action="{{ route('admin.empresas.categorias.destroy', $categoria) }}" method="POST" class="inline-block delete-form" onsubmit="return confirm('¿Estás seguro?');">
                                            @csrf @method('delete')
                ...
                {{-- Placeholder / Content Area when empty --}}
                 <div class="flex flex-col items-center justify-center py-12 px-4 text-center rounded-2xl bg-gray-50 dark:bg-gray-700/50 border-2 border-dashed border-gray-200 dark:border-gray-600">
                    <div class="bg-primary-50 dark:bg-primary-900/30 p-4 rounded-full mb-4">
                        <i class="fas fa-layer-group text-4xl text-primary-400"></i>
                    </div>
                    <h3 class="text-lg font-semibold text-gray-900 dark:text-white mb-2">No tienes categorías creadas</h3>
                    <p class="text-gray-500 dark:text-gray-400 max-w-sm mb-6">
                        Crea categorías para organizar tus cursos internos. Puedes hacerlas públicas si deseas compartirlas.
                    </p>
                    ...
                    <div x-show="openCreateModal" class="inline-block align-bottom bg-white dark:bg-gray-800 rounded-2xl text-left overflow-hidden shadow-xl transform transition-all sm:my-8 sm:align-middle sm:max-w-lg w-full">
                        <form action="{{ route('admin.empresas.categorias.store') }}" method="POST">
                            ...
                            <div class="bg-white dark:bg-gray-800 px-4 pt-5 pb-4 sm:p-6 sm:pb-4">
                                <h3 class="text-lg leading-6 font-medium text-gray-900 dark:text-white mb-4">Nueva Categoría</h3>
                                <div class="space-y-4">
                                    <div>
                                    ...
                                    <div class="flex items-center mt-4">
                                        ...
                                        <label for="create_public" class="ml-2 block text-sm text-gray-900 dark:text-gray-300">
                                            Hacer pública (Visible para otras empresas)
                                        </label>
                                    </div>
                                </div>
                            </div>
                ...
                <div class="flex items-end justify-center min-h-screen pt-4 px-4 pb-20 text-center sm:block sm:p-0">
                    ...
                    <div x-show="openEditModal" class="inline-block align-bottom bg-white dark:bg-gray-800 rounded-2xl text-left overflow-hidden shadow-xl transform transition-all sm:my-8 sm:align-middle sm:max-w-lg w-full">
                        <form :action="editAction" method="POST">
                            @csrf @method('PUT')
                            <div class="bg-white dark:bg-gray-800 px-4 pt-5 pb-4 sm:p-6 sm:pb-4">
                                <h3 class="text-lg leading-6 font-medium text-gray-900 dark:text-white mb-4">Editar Categoría</h3>
                                <div class="space-y-4">
                                    <div>
                                        <label class="block text-sm font-bold text-gray-700 dark:text-gray-300 mb-1">Nombre</label>
