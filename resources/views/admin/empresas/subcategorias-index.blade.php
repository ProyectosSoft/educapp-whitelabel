<x-admin-layout>
    <div class="px-4 py-8 md:px-8 space-y-8" x-data="{ openCreateModal: false, openEditModal: false, editAction: '', editName: '', editCategoryId: '', editPublic: false }">
        ...
        <div class="bg-primary rounded-[2rem] shadow-xl shadow-primary/20 flex flex-col sm:flex-row justify-between items-center p-8 relative overflow-hidden">
            ...
            <div class="relative z-10 flex items-center gap-5">
                <div class="bg-white/10 p-4 rounded-2xl backdrop-blur-md shadow-inner text-[#ECBD2D]">
                    <i class="fas fa-tags text-3xl drop-shadow-md"></i>
                </div>
                <div>
                   <h2 class="text-3xl font-extrabold text-white tracking-tight mb-1">Mis Subcategorías</h2>
                   <p class="text-blue-100 font-medium text-sm">Gestiona y organiza las subcategorías exclusivas para: <span class="text-[#ECBD2D] font-bold">{{ $empresa->nombre }}</span></p>
                </div>
            </div>
            ...
            <button @click="openCreateModal = true" class="relative z-10 mt-6 sm:mt-0 px-6 py-3.5 bg-[#ECBD2D] hover:bg-[#d4a827] text-primary-900 font-bold rounded-xl transition-all shadow-lg hover:shadow-[#ECBD2D]/40 hover:-translate-y-0.5 flex items-center group">
                <i class="fas fa-plus mr-2 transform group-hover:rotate-90 transition-transform duration-300"></i> Nueva Subcategoría
            </button>
        </div>
        ...
        <div class="bg-white rounded-3xl shadow-xl shadow-gray-200/50 border border-gray-100 overflow-hidden relative z-10">
                ...
                <table class="w-full text-left border-collapse">
                    <thead>
                        <tr class="bg-gray-50 border-b border-gray-100">
                            <th class="px-8 py-5 text-xs font-extrabold text-gray-500 uppercase tracking-wider">Nombre de la Subcategoría</th>
                            <th class="px-8 py-5 text-xs font-extrabold text-gray-500 uppercase tracking-wider">Categoría Padre</th>
                            ...
                            <tr class="hover:bg-gray-50/50 transition-colors group">
                                <td class="px-8 py-5">
                                    <div class="flex items-center">
                                        ...
                                        <div class="ml-4">
                                            <div class="font-bold text-gray-900 text-sm">{{ $subcategoria->nombre }}</div>
                                        </div>
                                    </div>
                                </td>
                                <td class="px-8 py-5">
                                    @if($subcategoria->Categoria)
                                        <span class="inline-flex items-center px-3 py-1.5 rounded-lg text-xs font-bold bg-blue-50 text-blue-600 border border-blue-100 shadow-sm">
                                            <i class="fas fa-layer-group mr-1.5 text-blue-400 opacity-70"></i>
                                            {{ $subcategoria->Categoria->nombre }}
                                        </span>
                                    @else
                                        <span class="inline-flex items-center px-3 py-1.5 rounded-lg text-xs font-bold bg-gray-50 text-gray-400 border border-gray-100 border-dashed">
                                            <i class="fas fa-unlink mr-1.5 opacity-50"></i>
                                            Sin Categoría
                                        </span>
                                    @endif
                                </td>
                                ...
                                <td class="px-8 py-5 text-right font-medium">
                                    <div class="flex items-center justify-end space-x-2">
                                        ...
                                        <form action="{{ route('admin.empresas.subcategorias.destroy', $subcategoria) }}" method="POST" onsubmit="return confirm('¿Estás seguro de eliminar esta subcategoría?');">
                                            @csrf @method('delete')
            ...
            <div class="flex flex-col items-center justify-center py-20 px-4 text-center">
                <div class="w-24 h-24 bg-primary-50 rounded-full flex items-center justify-center mb-6 shadow-inner">
                    <i class="fas fa-tags text-4xl text-primary-300"></i>
                </div>
                <h3 class="text-2xl font-bold text-gray-900 mb-2">No tienes subcategorías creadas</h3>
                <p class="text-gray-500 max-w-md mx-auto mb-8 font-medium">
                    Mantén todo organizado. Crea subcategorías personalizadas para agrupar mejor los temas dentro de tus cursos en <span class="font-bold text-gray-700">{{ $empresa->nombre }}</span>.
                </p>
                <button @click="openCreateModal = true" class="px-6 py-3 bg-primary text-white font-bold rounded-xl hover:bg-primary-600 transition-all shadow-lg shadow-primary/20 flex items-center transform hover:-translate-y-0.5">
                    <i class="fas fa-plus mr-2"></i> Crear Primera Subcategoría
                </button>
            </div>
            @endif
        </div>
            ...
            <div class="bg-white rounded-3xl shadow-2xl w-full max-w-md relative z-10 transform overflow-hidden" 
                ...
                <form action="{{ route('admin.empresas.subcategorias.store') }}" method="POST">
                    ...
                    <div class="p-8 space-y-6">
                        <div>
                            <label class="block text-sm font-bold text-gray-700 mb-2">Nombre de la Subcategoría</label>
                            <div class="relative group">
                                <div class="absolute inset-y-0 left-0 pl-4 flex items-center pointer-events-none">
                                    <i class="fas fa-tag text-gray-400 group-focus-within:text-primary transition-colors"></i>
                                </div>
                                <input type="text" name="nombre" required placeholder="Ej: Excel Avanzado" 
                        ...
                        <div>
                            <label class="block text-sm font-bold text-gray-700 mb-2">Categoría Padre</label>
                            <div class="relative group">
                                <div class="absolute inset-y-0 left-0 pl-4 flex items-center pointer-events-none">
                                    <i class="fas fa-layer-group text-gray-400 group-focus-within:text-primary transition-colors"></i>
                                </div>
                                <select name="categoria_id" required 
                                        class="pl-11 w-full rounded-xl border-gray-200 bg-gray-50 focus:bg-white text-gray-900 focus:border-primary focus:ring focus:ring-primary/20 py-3 shadow-sm transition appearance-none cursor-pointer">
                                    <option value="" disabled selected hidden>Selecciona la categoría origen...</option>
                        ...
                        <div class="p-4 rounded-xl border border-gray-100 bg-gray-50 flex items-start group hover:bg-white hover:shadow-sm transition-all cursor-pointer" @click="document.getElementById('create_public').click()">
                            ...
                            <div class="ml-3 text-sm">
                                ...
                                <p class="text-gray-500 font-medium">Permitir que otras empresas vean esta subcategoría.</p>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="p-8 space-y-6">
                        <div>
                         ...
                         <div>
                            ...
                            <div class="relative group">
                                <div class="absolute inset-y-0 left-0 pl-4 flex items-center pointer-events-none">
                                    <i class="fas fa-layer-group text-gray-400 group-focus-within:text-indigo-600 transition-colors"></i>
                                </div>
                                <select name="categoria_id" x-model="editCategoryId" required 
                                        class="pl-11 w-full rounded-xl border-gray-200 bg-gray-50 focus:bg-white text-gray-900 focus:border-indigo-500 focus:ring focus:ring-indigo-500/20 py-3 shadow-sm transition appearance-none cursor-pointer">
                                    <option value="" disabled hidden>Selecciona la categoría origen...</option>
                                    @foreach($categorias as $categoria)
                                        <option value="{{ $categoria->id }}">{{ $categoria->nombre }}</option>
                                    @endforeach
                                </select>
