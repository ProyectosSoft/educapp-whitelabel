<div class="space-y-4">
    @foreach ($course->Requerimiento_curso as $item)
        <article class="bg-white border border-gray-200 rounded-lg shadow-sm transition hover:shadow-md">
            <div class="px-5 py-4">
                @if ($requisito->id == $item->id)
                    <div class="animate-fade-in-down">
                         <textarea wire:model="requisito.nombre" class="form-input w-full bg-white border-gray-300 rounded-lg focus:ring-indigo-500 focus:border-indigo-500 p-2.5 transition text-sm" rows="2"></textarea>
                        @error('requisito.nombre')
        ...
        <div x-data="{ open: false }">
            ...
            <div x-show="open" style="display: none;" class="mt-3 bg-gray-50 border border-gray-200 rounded-lg p-4 animate-fade-in-up">
                 <form wire:submit.prevent="store">
                    <label class="block text-xs font-bold text-gray-500 uppercase mb-2">Nuevo Requisito</label>
                    <textarea wire:model="nombre" class="form-input w-full bg-white border-gray-300 rounded-lg focus:ring-indigo-500 focus:border-indigo-500 p-2.5 transition text-sm" rows="2" placeholder="Ej: Conocimientos básicos de programación..."></textarea>
